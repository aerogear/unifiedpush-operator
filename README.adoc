...
Images are automatically built and pushed to our https://quay.io/repository/aerogear/unifiedpush-operator[image repository] in the following cases:
...
If the image does not get built and pushed automatically the job may be re-run manually via the https://circleci.com/gh/aerogear/unifiedpush-operator[CI dashboard].
...
* Looking for the SOPs and update the tag for the them in all files (e.g `https://github.com/aerogear/unifiedpush-operator/blob/0.1.0/SOP/SOP-operator.adoc`)
...
NOTE: The image with the tag will be created and pushed to the https://quay.io/repository/aerogear/unifiedpush-operator[unifiedpush-operator image hosting repository] by the CI.
